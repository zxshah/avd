...
class ActionModule(ActionBase):
    def run(self, tmp: Any = None, task_vars: dict | None = None) -> dict:
        ...
        self.update_task_vars(task_vars, tmp_path)
    ...
    def update_task_vars(self, task_vars: dict, tmp_path: Path) -> None:
        """
        Read files generated by eos_designs_facts and inserts 'avd_switch_facts', 'avd_overlay_peers' and 'avd_topology_peers' into task_vars.

        Notice that avd_switch_facts is a defaultdict only reading the device facts on request.
        """
        avd_overlay_peers_file = tmp_path / "peer_facts" / "avd_overlay_peers.json"
        task_vars["avd_overlay_peers"] = read_json_file(avd_overlay_peers_file, "AVD Overlay Peers Facts")

        avd_topology_peers_file = tmp_path / "peer_facts" / "avd_topology_peers.json"
        task_vars["avd_overlay_peers"] = read_json_file(avd_topology_peers_file, "AVD Topology Peers Facts")

        task_vars["avd_switch_facts"] = AvdSwitchFactsDefaultDict(tmp_path)


def read_json_file(file: Path, file_context: str) -> dict | list:
    try:
