...
class FabricDocumentationFacts(AvdFacts):
    ...
    def __init__(  # pylint: disable=super-init-not-called
        self, peer_facts: dict[str, EosDesignsFacts], structured_configs: dict[str, dict], fabric_name: str, include_connected_endpoints: bool, toc: bool
    ) -> None:
    ...
    def fabric_switches(self) -> list[dict]:
        ...
        return natural_sort(
            [
                {
                    "node": hostname,
                    "type": self.peer_facts[hostname].type,
                    "pod": self.peer_facts[hostname].pod,
                    "mgmt_ip": self.peer_facts[hostname].mgmt_ip or "-",
                    "platform": self.peer_facts[hostname].platform or "-",
                    "provisioned": "Provisioned" if self.peer_facts[hostname].is_deployed else "Not Available",
                    "serial_number": self.peer_facts[hostname].serial_number or "-",
                    "inband_mgmt_ip": self.peer_facts[hostname].inband_mgmt_ip,
                    "inband_mgmt_interface": self.peer_facts[hostname].inband_mgmt_interface,
        ...
        for hostname, structured_config in self.structured_configs.items():
            for ethernet_interface in get(structured_config, "ethernet_interfaces", default=[]):
                ...
                if peer_type == "mlag_peer":
                    peer_type = self.peer_facts[peer].type
                    mlag_peer = True
                else:
                    mlag_peer = False
                ...
                data = (
                    self.peer_facts[hostname].type,  # type
    ...
    def uplink_ipv4_networks(self) -> list[dict]:
        ...
        pools_set = {facts.uplink_ipv4_pool for facts in self.peer_facts.values() if facts.uplink_ipv4_pool}
        ...
        ip_addresses = [IPv4Network(data[2], strict=False) for edge in self._topology.get_edges() for _, data in edge.node_data if data[2] is not None]
    ...
    def loopback_ipv4_networks(self) -> list[dict]:
        ...
        # Build set of loopback_ipv4_pool for all devices
        pools_set = {facts.loopback_ipv4_pool for facts in self.peer_facts.values() if facts.loopback_ipv4_pool}
        networks = [network for pool in pools_set for network in get_networks_from_pool(pool) if network.version == 4]
    ...
    def vtep_loopback_ipv4_networks(self) -> list[dict]:
        ...
        pools_set = {facts.vtep_loopback_ipv4_pool for facts in self.peer_facts.values() if facts.vtep_loopback_ipv4_pool}
        networks = [network for pool in pools_set for network in get_networks_from_pool(pool) if network.version == 4]
    ...
    def all_connected_endpoints(self) -> dict[str, list]:
        ...
        for hostname, structured_config in self.structured_configs.items():
            connected_endpoints_keys = self.peer_facts[hostname].connected_endpoints_keys
    ...
    def all_connected_endpoints_keys(self) -> list[dict]:
        ...
        set_of_tuples = {(item.key, item.type, item.description) for facts in self.peer_facts.values() for item in facts.connected_endpoints_keys}
        return natural_sort(
            [
                {
                    "key": key,
    ...
    def all_port_profiles(self) -> list[dict]:
        ...
        set_of_tuples = {(item.profile, item.parent_profile) for facts in self.peer_facts.values() for item in facts.port_profile_names}
        return natural_sort(
            [
                {
                    "profile": profile,
