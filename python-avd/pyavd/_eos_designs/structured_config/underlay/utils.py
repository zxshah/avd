...
class UtilsMixin(Protocol):
    ...
    def _underlay_vlan_trunk_groups(self: AvdStructuredConfigUnderlayProtocol) -> list:
        """Returns a list of dicts with vlans and trunk groups to configure on the underlay link."""
        ...
        for peer in self._avd_peers:
            ...
            for uplink in peer_facts.uplinks:
                if uplink.peer == self.shared_utils.hostname:
                    if not uplink.peer_trunk_groups:
                        continue

                    trunk_groups.append(
                        {
                            "vlan_list": uplink.vlans,
                            "trunk_groups": uplink.peer_trunk_groups,
                        },
                    )
