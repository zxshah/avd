...
| <samp>strict_system_mac_address</samp> | bool | optional | False |  | If `true`, raise an exception if the input structured configuration contains devices with a duplicated system_mac_address but unique serial_number values.<br>Otherwise, just issue a warning. |
