...
## Relationship to `arista.avd.eos_validate_state`
...
- **Tighter ANTA integration:** Direct access to ANTA's full capabilities and automatic adoption of new ANTA features and improvements.
...
- Add **all** [ANTA tests](https://anta.arista.com/stable/api/tests/) to the role 🚀

## Understanding ANTA Catalogs

The `anta_runner` role works with two types of test catalogs:

1. **AVD-generated catalogs:** These are automatically created based on the structured configurations of your devices. They contain tests tailored to the specific configuration of each device.
...
This playbook targets the Ansible inventory group `FABRIC`, so all devices under this group will be targeted for the ANTA tests.
...
Other optional filtering mechanisms are available to modify the behavior of the role.
...
!!! warning
    Tests in the AVD-generated catalogs are tagged with the device's hostname only. This means using `anta_runner_tags` with other tags than any device's hostname will skip all AVD-generated tests. To run/skip tests from the AVD-generated catalogs, use the `anta_runner_filters` variable as described below.

#### Test-Based Filtering

`anta_runner_filters`: Filters used to run or skip tests from the AVD-generated catalogs. These filters do **not** apply to user-defined catalogs, use `anta_runner_tags` for that. See the [AVD-generated catalog test index](#avd-generated-catalog-test-index) for the available tests.
